...
private:
    ...
    const double T_JALAN_TEMPAT    = 4.0;
    const double T_JALAN_MAJU      = 6.0;
...
public:
    bool initialize(SimpleControllerIO* io) override
    {
        ...
        // being applied.
        {
            ...
            std::cout << "[CTRL] all joints = JointDisplacement "
                      << "(Forward Dynamics with internal high-gain PD); "
                      << "torque applied by sim is logged below"
                      << std::endl;
        }
    ...
    bool control() override
    {
        ...
        if(!dispatched &&
           isActiveWindow(USE_JALAN_TEMPAT, cursor, T_JALAN_TEMPAT, localT)){
            nodeJalanDitempat(robot, localT, freezeX, freezeY);
            dispatched = true;
        }
        if(!dispatched &&
           isActiveWindow(USE_JALAN_MAJU, cursor, T_JALAN_MAJU, localT)){
            yAfterMaju = nodeJalanMaju(
                ...
                freezeX, rootYSign, walkSpeed, rootRampDur);
            dispatched = true;
        ...
        if(!dispatched &&
           isActiveWindow(USE_JALAN_MUNDUR, cursor, T_JALAN_MUNDUR, localT)){
            yAfterMundur = nodeJalanMundur(
                ...
                freezeX, yAfterMaju, rootYSign, walkSpeed, rootRampDur);
            dispatched = true;
        } else if(!dispatched){
        ...
        if(!dispatched &&
           isActiveWindow(USE_DIAM_AKHIR, cursor, T_DIAM_AKHIR, localT)){
            robot.setRoot(freezeX, yAfterMundur, 0.0, 0.0);
            dispatched = true;
        }
        if(!dispatched){
            robot.setRoot(freezeX, yAfterMundur, 0.0, 0.0);
